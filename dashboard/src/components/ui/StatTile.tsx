...
export function StatTile({
  ...
  return (
    <div className={clsx('panel flex flex-col gap-1 border-l-2 p-3', borderTones[tone], className)}>
      <div className="flex items-center justify-between gap-2">
        <span className="text-[0.65rem] font-semibold uppercase tracking-widest text-ink-faint">{label}</span>
        {icon && <span className="text-ink-faint">{icon}</span>}
      </div>
      <div className="flex items-baseline gap-1">
        <span className={clsx('tnum font-mono text-2xl font-bold leading-none', tones[tone])}>{value}</span>
